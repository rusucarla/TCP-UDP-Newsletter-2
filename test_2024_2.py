...
class Process:
  ...
  def start(self):
    ...
    try:
      if self.cwd == "":
        self.proc = Popen(self.comanda, universal_newlines=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
      else:
        self.proc = Popen(self.comanda, universal_newlines=True, stdin=PIPE, stdout=PIPE, stderr=PIPE, cwd=self.cwd)
      self.started = True
...
def subscribe_to_topic(c, topic_name="", wildcard=""):
  """Sends subscribe comanda on a client"""
